...
var edit=function(){var d,b=function(a){util.checkSuccess("Save node",a);view.refreshTree(null,!1);meta64.changePage("#mainPage");view.scrollToSelectedNode()},a=function(a){util.checkSuccess("Export",a)&&(alert("Export Successful."),meta64.changePage("#mainPage"),view.scrollToSelectedNode())},c=function(a){util.checkSuccess("Import",a)&&(alert("Import Successful."),view.refreshTree(null,!1),meta64.changePage("#mainPage"),view.scrollToSelectedNode())},f=function(a){console.log("insertBookResponse running.");util.checkSuccess("Insert Book",
a);view.refreshTree(null,!1);meta64.changePage("#mainPage");view.scrollToSelectedNode()},e=function(a){util.checkSuccess("Delete node",a)&&(meta64.clearSelectedNodes(),view.refreshTree(null,!1))},g=function(a){util.checkSuccess("Move nodes",a);k.nodesToMove=null;view.refreshTree(null,!1)},h=function(a){util.checkSuccess("Change node position",a);view.refreshTree(null,!1);meta64.changePage("#mainPage");view.scrollToSelectedNode()},l=function(a){util.checkSuccess("Insert node",a);meta64.newChildNodeId=
...
!1);meta64.changePage("#mainPage");view.scrollToSelectedNode()},saveNode:function(){k.editingUnsavedNode?k.saveNewNode():k.saveExistingNode()},saveNewNode:function(){var a=util.getRequiredElement("#newNodeNameId").val();console.log("Sending up first node name: "+a);meta64.treeDirty=!0;k.nodeInsertTarget?util.json("insertNode",{parentId:d.id,targetName:k.nodeInsertTarget.name,newNodeName:a},l):util.json("createSubNode",{nodeId:d.id,newNodeName:a},n)},saveExistingNode:function(){for(var a=[],c=0,f=
0;;){var e="editNodeTextContent"+c;if(meta64.fieldIdToPropMap.hasOwnProperty(e)){var d=meta64.fieldIdToPropMap[e],e=$("#"+e).val();e!==d.value&&(a.push({name:d.name,value:e}),f++)}else break;c++}0<f?util.json("saveNode",{nodeId:k.editNode.id,properties:a},b):alert("You didn't change any information!")},moveNodeUp:function(a){var b=meta64.uidToNodeMap[a];b?(a=meta64.getOrdinalOfNode(b),console.log("ordinal="+a),-1==a&&0>=a||util.json("setNodePosition",{parentNodeId:meta64.currentNodeId,nodeId:b.name,
siblingId:meta64.currentNodeData.children[a-1].name},h)):console.log("idToNodeMap does not contain "+a)},moveNodeDown:function(a){var b=meta64.uidToNodeMap[a];b?(a=meta64.getOrdinalOfNode(b),console.log("ordinal="+a),-1==a&&a>=meta64.currentNodeData.children.length-1||util.json("setNodePosition",{parentNodeId:meta64.currentNodeData.node.id,nodeId:meta64.currentNodeData.children[a+1].name,siblingId:b.name},h)):console.log("idToNodeMap does not contain "+a)},openExportPg:function(){meta64.changePage("#exportPg")},
...
{nodeId:a.id,sourceFileName:b},c)},runEditNode:function(a){var b=meta64.uidToNodeMap[a];b?(k.editingUnsavedNode=!1,k.editNode=b,meta64.changePage("#editNodePg")):(k.editNode=null,alert("Unknown nodeId in editNodeClick: "+a))},populateEditNodePg:function(){view.initEditPathDisplayById("#editNodePathDisplay");var a="",b=0;meta64.fieldIdToPropMap={};if(k.editNode)$.each(k.editNode.properties,function(c,f){if(render.allowPropertyToDisplay(f.name)){var e="editNodeTextContent"+b;meta64.fieldIdToPropMap[e]=
f;var d=f.values&&0<f.values.length,l=render.isReadOnlyProperty(f.name),h=render.isBinaryProperty(f.name);if(l||h)e="RdOnly_"+e;var g="";if(!l&&!h){var g=render.makeTag("a",{onClick:"props.clearProperty('"+e+"');","class":"ui-btn ui-btn-inline ui-icon-back ui-btn-icon-left"},"Clear"),m="",k="";"jcr:content"!==f.name&&(k=render.makeTag("a",{onClick:"props.deleteProperty('"+f.name+"');","class":"ui-btn ui-btn-inline ui-icon-delete ui-btn-icon-left"},"Del"),m=render.makeTag("a",{onClick:"props.addSubProperty('"+
e+"');","class":"ui-btn ui-btn-inline ui-icon-star ui-btn-icon-left"},"Add Multi"));g=m+g+k;g=0<g.length?render.makeHorizontalFieldSet(g):""}d?(console.log("Property multi-type: name="+f.name+" count="+f.values.length),g+=props.makeMultiPropEditor(e,f,l,h)):(console.log("Property single-type: "+f.name),g+=render.makeTag("label",{"for":e},render.sanitizePropertyName(f.name)),d=h?"[binary]":f.value,g=l||h?g+render.makeTag("textarea",{id:e,readonly:"readonly",disabled:"disabled"},d?d:""):g+render.makeTag("textarea",
...
k.nodeInsertTarget=null;k.startEditingNewNode()},deleteSelNodes:function(){var a=meta64.getSelectedNodeIdsArray();a&&0!=a.length?confirmPg.areYouSure("Confirm Delete","Delete "+a.length+" node(s) ?","Yes, delete.",function(){util.json("deleteNodes",{nodeIds:a},e)}):alert("You have not selected any nodes. Select nodes to delete first.")},moveSelNodes:function(){var a=meta64.getSelectedNodeIdsArray();a&&0!=a.length?confirmPg.areYouSure("Confirm Move","Move "+a.length+" node(s) to a new location ?",
...
readOnlyPropertyList:{"jcr:uuid":!0,"jcr:mixinTypes":!0,"jcr:created":!0,"jcr:createdBy":!0,"jcr:lastModified":!0,"jcr:lastModifiedBy":!0,imgWidth:!0,imgHeight:!0,binVer:!0,"jcr:mimeType":!0},binaryPropertyList:{"jcr:data":!0},fieldIdToPropMap:{},selectedNodes:{},newChildNodeId:"",currentNodeData:null,currentNode:null,currentNodeUid:null,currentNodeId:null,currentNodePath:null,inSimpleMode:function(){return b.editModeOption===b.MODE_SIMPLE},changePage:function(a){pageMgr.buildPage(a);$.mobile.pageContainer.pagecontainer("change",
...
f||(b.parentUidToFocusNodeMap[b.currentNodeUid]=a,e=a.uid+"_row",e=$("#"+e),util.changeOrAddClass(e,"inactive-row","active-row"));c&&view.scrollToSelectedNode()}},refreshAllGuiEnablement:function(){util.getPropertyCount(b.selectedNodes);b.getHighlightedNode();util.setEnablementByName("navHome",b.currentNode&&!nav.displayingHome());util.setEnablementByName("navUpLevel",b.currentNode&&nav.parentVisibleToUser());util.setEnablementByName("propsToggle",b.currentNode&&!b.isAnonUser);util.setEnablementByName("deleteProperty",
!b.isAnonUser);util.setEnablementByName("editMode",b.currentNode&&!b.isAnonUser);util.setVisibility("#menuButton",!b.isAnonUser)},defineActions:function(a){for(var c=0;c<arguments.length;c++){var f=arguments[c],e=f.name,d=f["function"];b.actionNameToObjMap[e]=f;if("function"!==typeof d)console.log("Function not found for action "+e);else{var h="#"+e+"Button";if(!util.hookClick(h,d)){console.log("Failed to hook button: "+e);break}(e=$(h))?util.setEnablement(e,f.enable):console.log("Unable to set enablement. ID not found: "+
h)}}},getSingleSelectedNode:function(){for(var a in b.selectedNodes)if(b.selectedNodes.hasOwnProperty(a))return b.uidToNodeMap[a];return null},getOrdinalOfNode:function(a){if(!b.currentNodeData||!b.currentNodeData.children)return-1;for(var c=0;c<b.currentNodeData.children.length;c++)if(a.id===b.currentNodeData.children[c].id)return c;return-1},setCurrentNodeData:function(a){b.currentNodeData=a;b.currentNode=a.node;b.currentNodeUid=a.node.uid;b.currentNodeId=a.node.id;b.currentNodePath=a.node.path},
anonPageLoadResponse:function(a){a.renderNodeResponse?(util.setVisibility("#mainNodeContent",!0),util.setVisibility("#mainNodeStatusBar",!0),render.renderPageFromData(a.renderNodeResponse),b.refreshAllGuiEnablement()):(util.setVisibility("#mainNodeContent",!1),util.setVisibility("#mainNodeStatusBar",!1),console.log("setting listview to: "+a.content),util.setHtmlEnhanced($("#listView"),a.content));render.renderMainPageControls()},initNode:function(a){a?(a.uid=util.getUidForId(b.identToUidMap,a.id),
...
a,b.deviceHeight=$(window).height(),b.screenSizeChange())},1500))},displaySignupMessage:function(){"ok"===$("#signupCodeResponse").text()&&alert("Signup complete. You may now login.")},screenSizeChange:function(){b.currentNodeData&&$.each(b.currentNodeData.children,function(a,b){b.imgId&&render.adjustImageSize(b)})},orientationHandler:function(a){},loadAnonPageHome:function(a){util.json("anonPageLoad",{ignoreUrl:a},b.anonPageLoadResponse)}};console.log("Module ready: meta64.js");return b}();console.log("running module: nav.js");
...
b("Move","moveSelNodesButton","edit.moveSelNodes();")+b("Finish Moving","finishMovingSelNodesButton","edit.finishMovingSelNodes();")+b("Export","openExportPgButton","edit.openExportPg();")+b("Import","openImportPgButton","edit.openImportPg();")+b("Delete","deleteSelNodesButton","edit.deleteSelNodes();"),f=d("Edit",f),e=b("Text Search","searchPgButton","srch.searchPg();")+b("Timeline","timelineButton","srch.timeline();"),e=d("Search",e),a=render.makeTag("div",{style:"margin: 0; min-width: 300px; position: fixed; top: 2em; left: 2em;",
"data-role":"collapsible-set"},a+f+e);util.setHtmlEnhanced($("#popupMenuPg"),a)},init:function(){var b=util.getPropertyCount(meta64.selectedNodes),f=meta64.getHighlightedNode(),e=!util.nullOrUndef(edit.nodesToMove)&&!a.isAnonUser;util.setEnablement($("#changePasswordPgButton"),!meta64.isAnonUser);util.setEnablement($("#accountPreferencesPgButton"),!meta64.isAnonUser);util.setEnablement($("#insertBookWarAndPeaceButton"),meta64.isAdminUser&&null!=f,meta64.isAdminUser);util.setEnablement($("#donatePgButton"),
!0);util.setEnablement($("#manageAttachmentsButton"),null!=f);util.setEnablement($("#editNodeSharingButton"),null!=f);util.setEnablement($("#moveSelNodesButton"),null!=f);util.setEnablement($("#finishMovingSelNodesButton"),e);util.setEnablement($("#openExportPgButton"),meta64.isAdminUser,meta64.isAdminUser);util.setEnablement($("#openImportPgButton"),meta64.isAdminUser,meta64.isAdminUser);util.setEnablement($("#deleteSelNodesButton"),b);util.setEnablement($("#searchPgButton"),null!=f);util.setEnablement($("#timelineButton"),
null!=f)}};console.log("Module ready: popupMenuPg.js");return a}();console.log("running module: prefsPg.js");
...
saveProperty:function(){var a=$("#addPropertyNameTextContent").val(),b=$("#addPropertyValueTextContent").val();util.json("saveProperty",{nodeId:edit.editNode.id,propertyName:a,propertyValue:b},d)},addSubProperty:function(a){a=meta64.fieldIdToPropMap[a];util.isObject(a.values)||(a.values=[],a.values.push(a.value),a.value=null);a.values.push("");edit.populateEditNodePg()},clearProperty:function(a){var b=$("#"+a);0!=b.length&&b.val("");for(var e=0;1E3>e;){b=$("#"+a+"_subProp"+e);if(0==b.length)break;
b.val("");e++}},deletePropertyFromLocalData:function(a){for(var b=0;b<edit.editNode.properties.length;b++)if(a===edit.editNode.properties[b].name){edit.editNode.properties.splice(b,1);break}},makeMultiPropEditor:function(a,b,e,d){var h="",l=b.values;l&&0!=l.length||(l=[],l.push(""));for(var m=0;m<l.length;m++){console.log("prop multi-val["+m+"]="+l[m]);var n=a+"_subProp"+m,h=h+render.makeTag("label",{"for":n},(0==m?b.name:"*")+"."+m),k=d?"[binary]":l[m],h=d||e?h+render.makeTag("textarea",{id:n,readonly:"readonly",
disabled:"disabled"},k?k:""):h+render.makeTag("textarea",{id:n},k?k:"")}return h},setPreferredPropertyOrder:function(a){if(a){var b=[];$.each(a,function(a,c){"jcr:content"===c.name?b.unshift(c):b.push(c)});return b}},renderProperties:function(a){if(a){var b="<table data-role='table' class='prop-table ui-responsive ui-shadow table-stripe property-text'>",d=0,b=b+"<thead><tr><th></th><th></th></tr></thead>",b=b+"<tbody>";$.each(a,function(a,c){if(render.allowPropertyToDisplay(c.name)){var l=render.isBinaryProperty(c.name);
...
var render=function(){var d,b={renderNodeContent:function(a,c,f,d,g){var h="";f&&meta64.editMode&&(h+="Path: "+b.formatPath(a.path),h+="<div>",a.createdBy&&(h+="<span class='"+(a.createdBy===meta64.userName?"created-by-me":"created-by-other")+"'>Created By: "+a.createdBy+"</span>"),h+="<span id='ownerDisplay"+a.uid+"'></span></div>");!c||!meta64.editMode||a.id===a.path&&f||(h+="ID: "+a.id+cnst.BR);d&&!f&&a.name&&(h+="Name: "+a.name+" [uid="+a.uid+"]");c="";h&&(c+=b.makeTag("div",{"class":"header-text"},
h));if(meta64.showProperties)(h=props.renderProperties(a.properties))&&(c+=h);else if(h=props.getNodeProperty("jcr:content",a))h=props.renderProperty(h),0<h.length&&(c+=b.makeTag("div",{"class":"jcr-content"},b.markdown(h)));g&&a.hasBinary&&(a.binaryIsImage?a=b.makeImageTag(a):(a=b.makeTag("a",{href:b.getUrlForNodeAttachment(a)},"[Download Attachment]"),a=b.makeTag("div",{"class":"binary-link"},a)),c=c.contains(cnst.INSERT_ATTACHMENT)?c.replaceAll(cnst.INSERT_ATTACHMENT,a):c+a);return c},renderNodeAsListItem:function(a,
c,f,d){var g=a.uid,h=a.id===meta64.newChildNodeId;d=0<c&&1<d;c=c<f-1;f=a.name.startsWith("rep:")||meta64.currentNodeData.node.path.contains("/rep:");f=meta64.isAdminUser||!f;var l=meta64.getHighlightedNode();!h&&l&&l.uid===g&&(h=!0);d=b.makeButtonBarHtml(g,d,c,f);return b.makeTag("div",{"class":"node-table-row"+(h?" active-row":" inactive-row"),onClick:"nav.clickOnNodeRow(this, '"+g+"');",id:g+"_row"},d+b.makeTag("div",{id:g+"_content"},b.renderNodeContent(a,!0,!0,!0,!0)))},makeButtonBarHtml:function(a,
c,f,d){var g=selButton=createSubNodeButton=editNodeButton=moveNodeUpButton=moveNodeDownButton=insertNodeButton="";b.nodeHasChildren(a)&&(g=b.makeTag("a",{onClick:"nav.openNode('"+a+"');","class":"ui-btn ui-btn-b ui-btn-inline ui-icon-plus ui-mini ui-btn-icon-left"},"Open"));meta64.editMode&&(selButton=b.makeTag("a",{id:a+"_sel",onClick:"nav.toggleNodeSel('"+a+"');","class":"ui-btn ui-btn-inline ui-icon-check ui-mini ui-btn-icon-left "+(meta64.selectedNodes[a]?"ui-btn-b":"ui-btn-a")},"Sel"),createSubNodeButton=
b.makeTag("a",{onClick:"edit.createSubNode('"+a+"');","class":"ui-btn ui-btn-inline ui-icon-star ui-mini ui-btn-icon-left"},"New"),insertNodeButton=b.makeTag("a",{onClick:"edit.insertNode('"+a+"');","class":"ui-btn ui-btn-inline ui-icon-bars ui-mini ui-btn-icon-left"},"Ins"));meta64.editMode&&d&&(editNodeButton=b.makeTag("a",{onClick:"edit.runEditNode('"+a+"');","class":"ui-btn ui-btn-inline ui-icon-edit ui-mini ui-btn-icon-left"},"Edit"),meta64.currentNode.childrenOrdered&&(c&&(moveNodeUpButton=
b.makeTag("a",{onClick:"edit.moveNodeUp('"+a+"');","class":"ui-btn ui-btn-inline ui-icon-arrow-u ui-mini ui-btn-icon-left"},"Up")),f&&(moveNodeDownButton=b.makeTag("a",{onClick:"edit.moveNodeDown('"+a+"');","class":"ui-btn ui-btn-inline ui-icon-arrow-d ui-mini ui-btn-icon-left"},"Dn"))));a=selButton+g+insertNodeButton+createSubNodeButton+editNodeButton+moveNodeUpButton+moveNodeDownButton;return 0<a.length?b.makeHorizontalFieldSet(a):""},makeHorizontalFieldSet:function(a){a=b.makeTag("fieldset",{"data-role":"controlgroup",
"data-type":"horizontal"},a);return b.makeTag("div",{"class":"ui-field-contain"},a)},makeHorzControlGroup:function(a){return b.makeTag("div",{"data-role":"controlgroup","data-type":"horizontal"},a)},makeRadioButton:function(a,c,f,d){return b.makeTag("input",{type:"radio",name:c,id:f,checked:d?"checked":"unchecked"},"",!0)+b.makeTag("label",{"for":f},a)},nodeHasChildren:function(a){var b=meta64.uidToNodeMap[a];if(b)return b.hasChildren;console.log("Unknown nodeId in nodeHasChildren: "+a);return!1},
formatPath:function(a){return meta64.isAdminUser?a:a.replaceAll("/root","")},markdown:function(a){d||(d=(new Markdown.getSanitizingConverter).makeHtml);return"<div data-ajax='false'>"+d(a)+"</div>"},renderMainPageControls:function(){var a="";0<srch.numSearchResults()&&(a+=render.makeTag("a",{onClick:"nav.showSearchPage();","class":"ui-btn ui-btn-inline ui-icon-search ui-btn-icon-left"},"Back to Search Results"));var b=0<a.length;b&&util.setHtmlEnhanced($("#mainPageControls"),a);util.setVisibility("#mainPageControls",
b)},renderPageFromData:function(a){var c=!1;a?c=!0:a=meta64.currentNodeData;if(a&&a.node){util.setVisibility("#listView",!0);b.renderMainPageControls();meta64.treeDirty=!1;c&&(meta64.uidToNodeMap={},meta64.idToNodeMap={},meta64.selectedNodes={},meta64.initNode(a.node),meta64.setCurrentNodeData(a));var d="",e=b.renderNodeContent(a.node,!0,!0,!0,!0);if(0<e.length){var g=a.node.uid,e=b.makeTag("div",{"class":"node-table-row inactive-row",onClick:"nav.clickOnNodeRow(this, '"+g+"');",id:g+"_row"},e);$("#mainNodeContent").show();
$("#mainNodeContent").html(e)}else $("#mainNodeContent").hide();view.updateStatusBar();b.renderMainPageControls();if(a.children){var h=a.children.length,l=0;$.each(a.children,function(a,e){var g=b.generateRow(a,e,c,h,l);0!=g.length&&(d+=g,l++)})}0!=d.length||meta64.isAnonUser||(d=b.makeTag("a",{onClick:"edit.createSubNode();","class":"ui-btn ui-btn-inline ui-icon-star ui-btn-icon-left"},"Create Content"));util.setHtmlEnhancedById("#listView",d);meta64.getHighlightedNode()||util.scrollToTop()}else util.setVisibility("#listView",
!1),$("#mainNodeContent").html("No default content is available.")},generateRow:function(a,c,d,e,g){if(meta64.isNodeBlackListed(c))return"";d&&meta64.initNode(c);g++;return b.renderNodeAsListItem(c,a,e,g)},getUrlForNodeAttachment:function(a){return postTargetUrl+"bin?nodeId="+encodeURIComponent(a.path)+"&ver="+a.binVer},adjustImageSize:function(a){var b=$("#"+a.imgId);if(b){var d=b.attr("width"),e=b.attr("height");a.width&&a.height&&(a.width>meta64.deviceWidth-50?(d=meta64.deviceWidth-50,e=d*a.height/
...
!1)},makeTag:function(a,b,d,e){"undefined"===typeof e&&(e=!0);var g="<"+a;b&&(g+=" ",$.each(b,function(a,b){g=b.contains("'")?g+(a+'="'+b+'" '):g+(a+"='"+b+"' ")}));return g=e?g+(">"+d+"</"+a+">"):g+"/>"},makeEditField:function(a,c){return b.makeTag("label",{"for":c},a)+b.makeTag("input",{type:"text",name:c,id:c},"",!0)},makePasswordField:function(a,c){return b.makeTag("label",{"for":c},a)+b.makeTag("input",{type:"password",name:c,id:c},"",!0)},makeButton:function(a,b,d,e){d="ui-btn ui-btn-inline ui-btn-"+
d;e&&(d+=" "+e);return render.makeTag("a",{id:b,"class":d},a)},makeBackButton:function(a,b,d){return render.makeTag("a",{id:b,"class":"ui-btn ui-icon-carat-l ui-btn-inline ui-btn-"+d,"data-rel":"back"},a)},allowPropertyToDisplay:function(a){return null==meta64.simpleModePropertyBlackList[a]},isReadOnlyProperty:function(a){return meta64.readOnlyPropertyList[a]},isBinaryProperty:function(a){return meta64.binaryPropertyList[a]},sanitizePropertyName:function(a){return"simple"===meta64.editModeOption?
"jcr:content"===a?"Content":a:a}};console.log("Module ready: render.js");return b}();console.log("running module: search.js");
...
b)},setEnablementByName:function(b,a,c){var f=meta64.actionNameToObjMap[b];if(f){if(f.enable=a,b=$("#"+b+"Button"))d.setEnablement(b,a),"undefined"===typeof c||c?d.setVisibility(b,!0):d.setVisibility(b,!1)}else alert("Unrecognized actionName: "+b)},setEnablement:function(b,a,c){"undefined"===typeof c||c?d.setVisibility(b,!0):d.setVisibility(b,!1);a&&b.removeClass("ui-state-disabled");a||b.addClass("ui-state-disabled")},hookSliderChanges:function(){for(var b=0;b<arguments.length;b++){var a=arguments[b]+
"SliderChange",c=window[a];"function"!==typeof c&&alert("Function not found: "+a);if(!hookSlider("#"+arguments[b]+"Slider",c)){alert("Failed to hook slider: "+arguments[b]);break}}},hookSlider:function(b,a){d.getRequiredElement(b).change(a);return!0},hookClick:function(b,a){$(b).on("click",a);return!0},anyEmpty:function(){for(var b=0;b<arguments.length;b++){var a=arguments[b];if(!a||0==a.length)return!0}return!1},changeOrAddClass:function(b,a,c){b=$(b);b.toggleClass(a,!1);b.toggleClass(c,!0)},changeOrAddClass__mine:function(b,
a,c){if(b.className&&0!=b.className.length){for(var d=b.className.split(" "),e="",g=!1,h=0;h<d.length;h++){var l=d[h];if(l===c){if(g)continue;g=!0}else if(l===a){if(g)continue;l=c;g=!0}0<e.length&&(e+=" ");e+=l}g||(0<e.length&&(e+=" "),e+=c);b.className=e}else console.log("no existing className is available on element, so assigning to "+c),b.className=c},verifyType:function(b,a,c){return typeof b!==a?(alert(c),!1):!0},setHtmlEnhancedById:function(b,a){var c=d.getRequiredElement(b);c?($(c).html(a),
$(c).enhanceWithin()):console.log("setting failed.")},setHtmlEnhanced:function(b,a){b.html(a);b.enhanceWithin()},getPropertyCount:function(b){var a=0,c;for(c in b)b.hasOwnProperty(c)&&a++;return a},printObject:function(b){var a="";$.each(b,function(b,d){a+=b+" , "+d+"\n"});return a},printProperties:function(b){var a="";$.each(b,function(b,d){a+=b+"\n"});return a},scrollToTop:function(){window.scrollTo(0,0)},printKeys:function(b){var a="";$.each(b,function(b,d){0<a.length&&(a+=",");a+=b});return a},
setVisibility:function(b,a){a?$(b).show():$(b).hide()}};console.log("Module ready: util.js");return d}();console.log("running module: view.js");
