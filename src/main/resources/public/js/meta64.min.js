...
var render=function(){var d,b={renderNodeContent:function(a,c,f,d,g){var h="";if(f&&meta64.editMode){var k=a.path,h=h+"<div>"+("<span class='path-display'>Path: "+b.formatPath(k)+"</span>");a.createdBy&&(h+="<span class='"+(a.createdBy===meta64.userName?"created-by-me":"created-by-other")+"'>Created By: "+a.createdBy+"</span>");h+="<span id='ownerDisplay"+a.uid+"'></span>";h+="</div>"}!c||!meta64.editMode||a.id===a.path&&f||(h+="ID: "+a.id+cnst.BR);d&&!f&&a.name&&(h+="Name: "+a.name+" [uid="+a.uid+
"]");c="";h&&(c+=b.makeTag("div",{"class":"header-text"},h));if(meta64.showProperties)(h=props.renderProperties(a.properties))&&(c+=h);else if(h=props.getNodeProperty("jcr:content",a))h=props.renderProperty(h),0<h.length&&(c+=b.makeTag("div",{"class":"jcr-content"},b.markdown(h)));g&&a.hasBinary&&(a.binaryIsImage?a=b.makeImageTag(a):(a=b.makeTag("a",{href:b.getUrlForNodeAttachment(a)},"[Download Attachment]"),a=b.makeTag("div",{"class":"binary-link"},a)),c=c.contains(cnst.INSERT_ATTACHMENT)?c.replaceAll(cnst.INSERT_ATTACHMENT,
