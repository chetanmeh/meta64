...
for(var d=0;d<b.currentNodeData.children.length;d++)if(a.id===b.currentNodeData.children[d].id)return d;return-1},setCurrentNodeData:function(a){b.currentNodeData=a;b.currentNode=a.node;b.currentNodeUid=a.node.uid;b.currentNodeId=a.node.id;b.currentNodePath=a.node.path},anonPageLoadResponse:function(a){a.renderNodeResponse?(util.setVisibility("#mainNodeContent",!0),util.setVisibility("#mainNodeStatusBar",!0),render.renderPageFromData(a.renderNodeResponse),b.refreshAllGuiEnablement()):(util.setVisibility("#mainNodeContent",
...
[jcrCnst.UUID,jcrCnst.MIXIN_TYPES,jcrCnst.CREATED,jcrCnst.CREATED_BY,jcrCnst.LAST_MODIFIED,jcrCnst.LAST_MODIFIED_BY,jcrCnst.IMG_WIDTH,jcrCnst.IMG_HEIGHT,jcrCnst.BIN_VER,jcrCnst.BIN_DATA,jcrCnst.BIN_MIME]);util.addAll(b.binaryPropertyList,[jcrCnst.BIN_DATA])},initApp:function(){c||(console.log("initApp running."),c=!0,b.initConstants(),b.displaySignupMessage(),$(window).on("orientationchange",b.orientationHandler),$(window).bind("beforeunload",function(){return"Leave Meta64 ?"}),$(window).on("unload",
...
"Edit"),l=c.makeHorizontalFieldSet(l,"compact-field-contain page-top-button-bar"));e=c.makeTag("div",{"class":"node-table-row page-parent-node inactive-row",onClick:"nav.clickOnNodeRow(this, '"+g+"');",id:k,style:e},l+f);$("#mainNodeContent").show();$("#mainNodeContent").html(e)}else $("#mainNodeContent").hide();$("#mainNodeContent").trigger("updatelayout");view.updateStatusBar();c.renderMainPageControls();if(b.children){var m=b.children.length,n=0;$.each(b.children,function(b,e){var f=c.generateRow(b,
e,a,m,n);0!=f.length&&(d+=f,n++)})}0!=d.length||meta64.isAnonUser||(d=c.makeTag("a",{onClick:"edit.createSubNode();","class":"ui-btn ui-btn-inline ui-icon-star ui-btn-icon-left"},"Create Content"));util.setHtmlEnhancedById("#listView",d);meta64.screenSizeChange();meta64.getHighlightedNode()||util.scrollToTop()}else util.setVisibility("#listView",!1),$("#mainNodeContent").html("No default content is available.")},generateRow:function(b,a,d,e,f){if(meta64.isNodeBlackListed(a))return"";d&&meta64.initNode(a);
...
a.homeNodeOverride,view.refreshTree(d,!1),c(a)):f&&(alert("Cookie login failed."),$.removeCookie(cnst.COOKIE_LOGIN_USR),$.removeCookie(cnst.COOKIE_LOGIN_PWD),$.writeCookie(cnst.COOKIE_LOGIN_STATE,"0"),location.reload())},d=function(a){window.location.href=window.location.origin},e=function(a){util.checkSuccess("Change password",a)&&alert("Password changed successfully.")},f=function(a){util.checkSuccess("Signup new user",a)&&(user.populateLoginPgFromCookies(),meta64.changePage(loginPg),alert("User Information Accepted. \n\nCheck your email for signup confirmation. (Can take up to 1 minute)"))},
g={twitterLogin:function(){$(window).off("beforeunload");window.location.href=window.location.origin+"/twitterLogin"},openSignupPg:function(){meta64.changePage(signupPg)},writeCookie:function(a,b){$.cookie(a,b,{expires:365,path:"/"})},populateLoginPgFromCookies:function(){var a=$.cookie(cnst.COOKIE_LOGIN_USR),b=$.cookie(cnst.COOKIE_LOGIN_PWD);a&&$("#userName").val(a);b&&$("#password").val(b)},openLoginPg:function(){meta64.isAnonUser?meta64.changePage(loginPg):g.logout(!0)},signup:function(){var a=
util.getRequiredElement("#signupUserName").val(),b=util.getRequiredElement("#signupPassword").val(),d=util.getRequiredElement("#signupEmail").val(),c=util.getRequiredElement("#signupCaptcha").val();util.anyEmpty(a,b,d,c)?alert("Sorry, you cannot leave any fields blank."):util.json("signup",{userName:a,password:b,email:d,captcha:c},f)},pageInitSignupPg:function(){user.tryAnotherCaptcha()},tryAnotherCaptcha:function(){var a=util.currentTimeMillis(),a=postTargetUrl+"captcha?t="+a;$("#captchaImage").attr("src",
a)},refreshLogin:function(){var d,e,f=!1;if("true"===$("#loginSessionReady").text())e=d="",f=!0;else{if("0"===$.cookie(cnst.COOKIE_LOGIN_STATE)){meta64.loadAnonPageHome(!1);return}var g=$.cookie(cnst.COOKIE_LOGIN_USR),p=$.cookie(cnst.COOKIE_LOGIN_PWD),f=!util.emptyString(g)&&!util.emptyString(p);d=g?g:"";e=p?p:""}console.log("refreshLogin with name: "+d);g=util.json("login",{userName:d,password:e,tzOffset:(new Date).getTimezoneOffset(),dst:util.daylightSavingsTime});f?g.done(function(b){a(b,d,e,f)}):
g.done(function(a){b(a);c(a);meta64.loadAnonPageHome(!1)})},login:function(){var b=$.trim($("#userName").val()),d=$.trim($("#password").val());util.json("login",{userName:b,password:d,tzOffset:(new Date).getTimezoneOffset(),dst:util.daylightSavingsTime}).done(function(c){a(c,b,d)})},logout:function(a){meta64.isAnonUser||($(window).off("beforeunload"),a&&user.writeCookie(cnst.COOKIE_LOGIN_STATE,"0"),util.json("logout",{},d))},changePassword:function(){var a=util.getRequiredElement("#changePassword1").val(),
b=util.getRequiredElement("#changePassword2").val();a&&4<=a.length&&a===b?util.json("changePassword",{newPassword:a},e):alert("Sorry, invalid password(s).")},changePasswordPg:function(){meta64.changePage(changePasswordPg)}};console.log("Module ready: user.js");return g}();console.log("running module: util.js");
...
b);if(!hookSlider("#"+arguments[a]+"Slider",c)){alert("Failed to hook slider: "+arguments[a]);break}}},hookSlider:function(a,c){b.getRequiredElement(a).change(c);return!0},bindEnterKey:function(a,c){b.bindKey(a,c,13)},bindKey:function(a,b,c){$(a).keypress(function(a){if(a.which==c)return b(),!1})},anyEmpty:function(){for(var a=0;a<arguments.length;a++){var b=arguments[a];if(!b||0==b.length)return!0}return!1},changeOrAddClass:function(a,b,c){a=$(a);a.toggleClass(b,!1);a.toggleClass(c,!0)},changeOrAddClass__mine:function(a,
b,c){if(a.className&&0!=a.className.length){for(var f=a.className.split(" "),g="",k=!1,l=0;l<f.length;l++){var m=f[l];if(m===c){if(k)continue;k=!0}else if(m===b){if(k)continue;m=c;k=!0}0<g.length&&(g+=" ");g+=m}k||(0<g.length&&(g+=" "),g+=c);a.className=g}else console.log("no existing className is available on element, so assigning to "+c),a.className=c},verifyType:function(a,b,c){return typeof a!==b?(alert(c),!1):!0},setHtmlEnhancedById:function(a,c){var e=b.getRequiredElement(a);e?($(e).html(c),
$(e).enhanceWithin()):console.log("setting failed.")},setHtmlEnhanced:function(a,b){a.html(b);a.enhanceWithin()},getPropertyCount:function(a){var b=0,c;for(c in a)a.hasOwnProperty(c)&&b++;return b},printObject:function(a){var b="";$.each(a,function(a,c){b+=a+" , "+c+"\n"});return b},printProperties:function(a){if(a){var b="";$.each(a,function(a,c){b+=a+"\n"});return b}console.error("printProperties recieved null.")},scrollToTop:function(){window.scrollTo(0,0)},printKeys:function(a){var b="";$.each(a,
...
$("html, body").animate({scrollTop:b})}},1E3)},initEditPathDisplayById:function(b){var a=edit.editNode;b=$(b);if(edit.editingUnsavedNode)b.html(""),b.hide();else{var c="Path: "+render.formatPath(a),c=c+("<br>ID: "+a.id),c=c+("<br>Modified: "+a.lastModified);b.html(c);b.show()}b.trigger("updatelayout")}};console.log("Module ready: view.js");return c}();
