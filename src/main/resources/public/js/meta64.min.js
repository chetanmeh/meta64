...
(b=!0);meta64.treeDirty=!0;l.nodeInsertTarget?util.json("insertNode",{parentId:c.id,targetName:l.nodeInsertTarget.name,newNodeName:a},n):util.json("createSubNode",{nodeId:c.id,newNodeName:a},q)},saveExistingNode:function(){for(var d=[],e=0,g=0;;){var c="editNodeTextContent"+e;if(meta64.fieldIdToPropMap.hasOwnProperty(c)){var h=meta64.fieldIdToPropMap[c],c=$("#"+c).val();c!==h.value&&(d.push({name:h.name,value:c}),g++)}else break;e++}0<g?(util.json("saveNode",{nodeId:l.editNode.id,properties:d,sendNotification:b},
...
e;var g=e.values&&0<e.values.length,h=render.isReadOnlyProperty(e.name),f=render.isBinaryProperty(e.name);if(h||f)c="RdOnly_"+c;var k="";if(!h&&!f){var k=render.makeTag("a",{onClick:"props.clearProperty('"+c+"');","class":"ui-btn ui-btn-inline ui-icon-back ui-btn-icon-left"},"Clear"),l="",m="";e.name!==jcrCnst.CONTENT&&(m=render.makeTag("a",{onClick:"props.deleteProperty('"+e.name+"');","class":"ui-btn ui-btn-inline ui-icon-delete ui-btn-icon-left"},"Del"),l=render.makeTag("a",{onClick:"props.addSubProperty('"+
c+"');","class":"ui-btn ui-btn-inline ui-icon-star ui-btn-icon-left"},"Add Multi"));k=l+k+m;k=0<k.length?render.makeHorizontalFieldSet(k):""}g?(console.log("Property multi-type: name="+e.name+" count="+e.values.length),k+=props.makeMultiPropEditor(c,e,h,f)):(console.log("Property single-type: "+e.name),k+=render.makeTag("label",{"for":c},render.sanitizePropertyName(e.name)),g=f?"[binary]":e.value,k=h||f?k+render.makeTag("textarea",{id:c,readonly:"readonly",disabled:"disabled"},g?g:""):k+render.makeTag("textarea",
{id:c},g?g:""));a+=render.makeTag("div",{"class":"ui-field-contain"},k);d++}else console.log("Hiding property: "+e.name)});else var b=render.makeTag("label",{"for":"newNodeNameId"},"New Node Name")+render.makeTag("textarea",{id:"newNodeNameId"},""),a=a+render.makeTag("div",{"class":"ui-field-contain"},b);util.setHtmlEnhanced($("#propertyEditFieldContainer"),a);b=l.editingUnsavedNode?"You may leave this field blank and a unique ID will be assigned. You only need to provide a name if you want this node to have a more meaningful URL.":
...
b+a),b=render.makeTag("div",{role:"main","class":"ui-content dialog-content"},""+b),c=c+b;util.setHtmlEnhanced($("#loginPg"),c);$("#loginButton").on("click",user.login);util.bindEnterKey("#userName",user.login);util.bindEnterKey("#password",user.login)},init:function(){util.delayedFocus("#userName")}}}();console.log("running module: popupMenuPg.js");
...
d=render.makeBackButton("Cancel","cancelPreferencesPgButton","a"),a=render.makeHorzControlGroup(a+d),b=render.makeTag("div",{"class":"ui-field-contain"},b+a),b=render.makeTag("div",{role:"main","class":"ui-content dialog-content"},""+b),c=c+b;util.setHtmlEnhanced($("#prefsPg"),c);$("#savePreferencesButton").on("click",prefs.savePreferences)}}}();console.log("running module: renameNodePg.js");
...
"signupButton","b","ui-btn-icon-left ui-icon-check"),h=render.makeButton("Try Different Image","tryAnotherCaptchaButton","a"),f=render.makeBackButton("Close","cancelSignupButton","a"),e=render.makeHorzControlGroup(e+h+f),a=render.makeTag("div",{"class":"ui-field-contain"},a+d+e),a=render.makeTag("div",{role:"main","class":"ui-content dialog-content",id:c.domId+"-main"},""+a),b=b+a;util.setHtmlEnhanced($("#signupPg"),b);$("#tryAnotherCaptchaButton").on("click",user.tryAnotherCaptcha);$("#signupButton").on("click",
...
meta64.MODE_ADVANCED:meta64.MODE_SIMPLE},a=function(a,d,e,h){util.checkSuccess("Login",a)?("anonymous"!=d&&(f.writeCookie(cnst.COOKIE_LOGIN_USR,d),f.writeCookie(cnst.COOKIE_LOGIN_PWD,e)),meta64.jqueryChangePage("#mainPage"),b(a),a.userPreferences.lastNode?console.log("lastNode: "+a.userPreferences.lastNode):console.log("lastNode is null."),d=null,d=util.emptyString(a.homeNodeOverride)?a.userPreferences.lastNode?a.userPreferences.lastNode:meta64.homeNodeId:a.homeNodeOverride,view.refreshTree(d,!1),
c(a)):h&&(alert("Cookie login failed."),$.removeCookie(cnst.COOKIE_LOGIN_USR),$.removeCookie(cnst.COOKIE_LOGIN_PWD),location.reload())},d=function(a){meta64.jqueryChangePage("#mainPage");location.reload()},e=function(a){util.checkSuccess("Change password",a)&&alert("Password changed successfully.")},h=function(a){util.checkSuccess("Signup new user",a)&&(user.populateLoginPgFromCookies(),meta64.changePage(loginPg),alert("User Information Accepted. \n\nCheck your email for signup confirmation. (Can take up to 1 minute)"))},
