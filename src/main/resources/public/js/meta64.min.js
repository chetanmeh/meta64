...
createSubNode:function(a){if(a){if(c=meta64.uidToNodeMap[a],!c){console.log("Unknown nodeId in createSubNode: "+a);return}}else c=meta64.currentNode;l.nodeInsertTarget=null;l.startEditingNewNode()},clearSelections:function(){meta64.clearSelectedNodes();render.renderPageFromData();meta64.jqueryChangePage("#mainPage")},deleteSelNodes:function(){var a=meta64.getSelectedNodeIdsArray();a&&0!=a.length?confirmPg.areYouSure("Confirm Delete","Delete "+a.length+" node(s) ?","Yes, delete.",function(){util.json("deleteNodes",
{nodeIds:a},g)}):alert("You have not selected any nodes. Select nodes to delete first.")},moveSelNodes:function(){var a=meta64.getSelectedNodeIdsArray();a&&0!=a.length?confirmPg.areYouSure("Confirm Move","Move "+a.length+" node(s) to a new location ?","Yes, move.",function(){l.nodesToMove=a;meta64.selectedNodes={};alert("Ok, ready to move nodes. To finish moving, go select the target location, then click 'Finish Moving'");meta64.refreshAllGuiEnablement()}):alert("You have not selected any nodes. Select nodes to delete first.")},
...
f):alert("No node is selected.")})}};console.log("Module ready: edit.js");return l}();console.log("running module: jcrCnst.js");
...
$("#navHomeButton").on("click",nav.navHome);$("#navUpLevelButton").on("click",nav.navUpLevel);$("#deletePropertyButton").on("click",props.deleteProperty);$("#editModeButton").on("click",edit.editMode)},openDonatePg:function(){meta64.jqueryChangePage("#donatePg")},getHighlightedNode:function(){return b.parentUidToFocusNodeMap[b.currentNodeUid]},highlightRowById:function(a,d){var e=b.getNodeFromId(a);e?b.highlightNode(e,d):console.log("highlightRowById failed to find id: "+a)},highlightNode:function(a,
d){if(a){var e=!1,c=b.parentUidToFocusNodeMap[b.currentNodeUid];c&&(c.uid===a.uid?e=!0:(c=c.uid+"_row",c=$("#"+c),util.changeOrAddClass(c,"active-row","inactive-row")));e||(b.parentUidToFocusNodeMap[b.currentNodeUid]=a,c=a.uid+"_row",c=$("#"+c),util.changeOrAddClass(c,"inactive-row","active-row"));d&&view.scrollToSelectedNode()}},refreshAllGuiEnablement:function(){util.getPropertyCount(b.selectedNodes);var a=b.getHighlightedNode();util.setEnablement($("#navHomeButton"),b.currentNode&&!nav.displayingHome());
...
b.isAnonUser);util.setVisibility("#mainMenuSearchButton",!b.isAnonUser&&null!=a);util.setVisibility("#mainMenuTimelineButton",!b.isAnonUser&&null!=a);util.setVisibility("#mainNavBar",!b.isAnonUser)},getSingleSelectedNode:function(){for(var a in b.selectedNodes)if(b.selectedNodes.hasOwnProperty(a))return b.uidToNodeMap[a];return null},getOrdinalOfNode:function(a){if(!b.currentNodeData||!b.currentNodeData.children)return-1;for(var d=0;d<b.currentNodeData.children.length;d++)if(a.id===b.currentNodeData.children[d].id)return d;
...
jcrCnst.LAST_MODIFIED,jcrCnst.LAST_MODIFIED_BY,jcrCnst.IMG_WIDTH,jcrCnst.IMG_HEIGHT,jcrCnst.BIN_VER,jcrCnst.BIN_DATA,jcrCnst.BIN_MIME]);util.addAll(b.binaryPropertyList,[jcrCnst.BIN_DATA])},initApp:function(){c||(console.log("initApp running."),c=!0,b.initConstants(),b.displaySignupMessage(),$(window).on("orientationchange",b.orientationHandler),$(window).bind("beforeunload",function(){return"Leave Meta64 ?"}),$(window).on("unload",function(){user.logout()}),b.addClickListeners(),b.deviceWidth=$(window).width(),
b.deviceHeight=$(window).height(),user.refreshLogin(),setInterval(function(){var a=$(window).width();a!=b.deviceWidth&&(console.log("Screen width changed: "+a),b.deviceWidth=a,b.deviceHeight=$(window).height(),b.screenSizeChange())},1500),b.refreshAllGuiEnablement())},displaySignupMessage:function(){"ok"===$("#signupCodeResponse").text()&&alert("Signup complete. You may now login.")},screenSizeChange:function(){b.currentNodeData&&(meta64.currentNode.imgId&&render.adjustImageSize(meta64.currentNode),
...
b+a),b=render.makeTag("div",{role:"main","class":"ui-content dialog-content"},""+b),c=c+b;util.setHtmlEnhanced($("#loginPg"),c);$("#loginButton").on("click",user.login);util.bindEnterKey("#userName",user.login);util.bindEnterKey("#password",user.login)},init:function(){util.delayedFocus("#userName")}}}();console.log("running module: messagePg.js");
var messagePg=function(){var c,b,a;console.log("Module ready: messagePg.js");return{domId:"messagePg",showMessage:function(d,e,h){c=d;b=e;a=h;meta64.changePage(messagePg)},build:function(){var a;a="<h3 id='messagePgTitle'></h3><p id='messagePgMessage'></p>"+render.makeBackButton("Ok","messagePgOkButton","b");a=render.makeTag("div",{"data-role":"content"},a);util.setHtmlEnhanced($("#messagePg"),a)},init:function(){$("#messagePgTitle").text(c);$("#messagePgMessage").html(b);$("#messagePgOkButton").off("click");
...
b("Move","moveSelNodesButton","edit.moveSelNodes();")+b("Finish Moving","finishMovingSelNodesButton","edit.finishMovingSelNodes();")+b("Rename","renameNodePgButton","edit.openRenameNodePg();")+b("Delete","deleteSelNodesButton","edit.deleteSelNodes();")+b("Clear Selections","clearSelectionsButton","edit.clearSelections();")+b("Import","openImportPgButton","edit.openImportPg();")+b("Export","openExportPgButton","edit.openExportPg();"),e=c("Edit",e),h=b("Toggle Properties","propsToggleButton","props.propsToggle();")+
b("Refresh","refreshPageButton","view.refreshPage();")+b("Show URL","showFullNodeUrlButton","render.showNodeUrl();"),h=c("View Options",h),a=render.makeTag("div",{style:"margin: 0; min-width: 300px; position: fixed; top: 2em; left: 2em;","data-role":"collapsible-set"},a+e+h);util.setHtmlEnhanced($("#popupMenuPg"),a)},init:function(){var d=util.getPropertyCount(meta64.selectedNodes),b=meta64.getHighlightedNode(),c=!util.nullOrUndef(edit.nodesToMove)&&!a.isAnonUser;util.setEnablement($("#changePasswordPgButton"),
...
null!=b&&!meta64.isAnonUser);util.setEnablement($("#finishMovingSelNodesButton"),c,c);util.setEnablement($("#openExportPgButton"),meta64.isAdminUser,meta64.isAdminUser);util.setEnablement($("#openImportPgButton"),meta64.isAdminUser,meta64.isAdminUser);util.setEnablement($("#deleteSelNodesButton"),d)}};console.log("Module ready: popupMenuPg.js");return a}();console.log("running module: prefsPg.js");
...
!1;a?d=!0:a=meta64.currentNodeData;if(a&&a.node){util.setVisibility("#listView",!0);b.renderMainPageControls();meta64.treeDirty=!1;d&&(meta64.uidToNodeMap={},meta64.idToNodeMap={},meta64.selectedNodes={},meta64.initNode(a.node),meta64.setCurrentNodeData(a));var c="",h=b.getNodeBkgImageStyle(a.node),f=b.renderNodeContent(a.node,!0,!0,!0,!0);if(0<f.length){var g=a.node.uid,k=g+"_row",m="";meta64.editMode&&(m=b.makeTag("a",{onClick:"edit.runEditNode('"+g+"');","class":"ui-btn ui-btn-inline ui-icon-edit ui-mini ui-btn-icon-left"},
"Edit"),m=b.makeHorizontalFieldSet(m,"compact-field-contain page-top-button-bar"));h=b.makeTag("div",{"class":"node-table-row page-parent-node inactive-row",onClick:"nav.clickOnNodeRow(this, '"+g+"');",id:k,style:h},m+f);$("#mainNodeContent").show();$("#mainNodeContent").html(h)}else $("#mainNodeContent").hide();$("#mainNodeContent").trigger("updatelayout");view.updateStatusBar();b.renderMainPageControls();if(a.children){var n=a.children.length,p=0;$.each(a.children,function(a,g){var k=b.generateRow(a,
g,d,n,p);0!=k.length&&(c+=k,p++)})}0!=c.length||meta64.isAnonUser||(c=b.makeTag("a",{onClick:"edit.createSubNode();","class":"ui-btn ui-btn-inline ui-icon-star ui-btn-icon-left"},"Create Content"));util.setHtmlEnhancedById("#listView",c);meta64.screenSizeChange();meta64.getHighlightedNode()||util.scrollToTop()}else util.setVisibility("#listView",!1),$("#mainNodeContent").html("No default content is available.")},generateRow:function(a,d,c,h,f){if(meta64.isNodeBlackListed(d))return"";c&&meta64.initNode(d);
f++;return b.renderNodeAsListItem(d,a,h,f)},getUrlForNodeAttachment:function(a){return postTargetUrl+"bin/file-name?nodeId="+encodeURIComponent(a.path)+"&ver="+a.binVer},adjustImageSize:function(a){var b=$("#"+a.imgId);b&&a.width&&a.height&&(a.width>meta64.deviceWidth-80?(b.attr("width","100%"),b.attr("height","auto")):(b.attr("width",a.width),b.attr("height",a.height)))},makeImageTag:function(a){var d=b.getUrlForNodeAttachment(a);a.imgId="imgUid_"+a.uid;if(a.width&&a.height){if(a.width>meta64.deviceWidth-
50){var c=meta64.deviceWidth-50;return b.makeTag("img",{src:d,id:a.imgId,width:c+"px",height:c*a.height/a.width+"px"},null,!1)}return b.makeTag("img",{src:d,id:a.imgId,width:a.width+"px",height:a.height+"px"},null,!1)}return b.makeTag("img",{src:d,id:a.imgId},null,!1)},makeTag:function(a,b,c,h){"undefined"===typeof h&&(h=!0);var f="<"+a;b&&(f+=" ",$.each(b,function(a,b){f=b.contains("'")?f+(a+'="'+b+'" '):f+(a+"='"+b+"' ")}));return f=h?f+(">"+c+"</"+a+">"):f+"/>"},makeEditField:function(a,d){return b.makeTag("label",
{"for":d},a)+b.makeTag("input",{type:"text",name:d,id:d},"",!0)},makePasswordField:function(a,d){return b.makeTag("label",{"for":d},a)+b.makeTag("input",{type:"password",name:d,id:d},"",!0)},makeButton:function(a,b,c,h){c="ui-btn ui-btn-inline ui-btn-"+c;h&&(c+=" "+h);return render.makeTag("a",{id:b,"class":c},a)},makeBackButton:function(a,b,c){return render.makeTag("a",{id:b,"class":"ui-btn ui-icon-carat-l ui-btn-inline ui-btn-"+c,"data-rel":"back"},a)},allowPropertyToDisplay:function(a){return null==
